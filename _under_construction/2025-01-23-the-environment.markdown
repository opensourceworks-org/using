...
A few high cloud bills ago, I decided to revisit the homelab thing. Having abandonned it at the time for reasons of maintenance and running cost, I decided to consolidate my running services on a few Raspberry Pis and use the cloud providers like GCP and AWS for my workload experimentation. Since a while, there has been a huge price drop of the Intel Haswell (E5 2600 v3) and Broadwell (E5 2600 v4) microarchitectures. Even gamers have picked those up in both single and dual socket layouts as budget friendly gaming rigs. To me, that sounds a bit enthousiastic, since gaming prefers better single core performance over multi core performance. However, these processors have decent L3 cache, so maybe that has something to do with it. Anyway, data processing using frameworks like spark and ML workloads definitely prefer multi core with loads or memory.

I was able to source a pretty decent used starting platform online for cheap, consisting of a dual socket motherboard, with E5 2620 v4 CPUs, 128GB of DDR4 2333MHz memory, nicely spread over the available memory lanes. It came in a nice case with 1000W PSU. No GPU apart from the Aspeed onboard graphics, and no disks. No problem. Found a duo refurbished E5 2699 v3 (18cores/36threads each), paired that with an RTX 2060 nvidia GPU, a PCIe x16 NVMe interface, a 2TB WD NVMe drive, 6 2TB WD HDD spindles (RAID10), an additional 4TB WD HDD (red) and a few SATA ssd drives as well. The Asus z10pe-d16 motherboard has plenty of interface options, including an onboard RAID controller.

Something that surprised me though, is how many people try to sell their not-so-well working computers and parts as fully operational and in "as good as new" state.
...
Then there was the cpu throttling. Using btop showed that at load the cpus were reaching 90°C, critical zone. There was a fan missing at the rear, so I ordered one. Then I had a good look at the case and could feel a lot of heat at the center of the case. The top fans were placed as intake instead of exhaust, and the cpu fans were oriented downwards, effectively blowing in the center. The airflow from the top intake was immediately sucked out the back, an airflow shortcircuit. Fine, flipped the 3 top fans, rotated both cpu heatsinks with fan 180° and bingo.. At 100% load, the temperatures stabilized at around a comfortable 72°C.
